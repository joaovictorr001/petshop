...
def filtrar_por_nome(produtos, nome):
    return [produto for produto in produtos if nome.lower() in produto["product_name"].lower()]

def filtrar_por_descricao(produtos, descricao):
    return [produto for produto in produtos if descricao.lower() in produto["product_description"].lower()]
...
def listar_produtos():
    ...
    if nome and descricao:
        lista_nome = filtrar_por_nome(products, nome)
        lista_nome_descricao = filtrar_por_descricao(lista_nome, descricao)
        return jsonify(lista_nome_descricao)

    if nome:
        lista_nome = filtrar_por_nome(products, nome)
        return jsonify(lista_nome)

    if descricao:
        lista_descricao = filtrar_por_descricao(products, descricao)
        return jsonify(lista_descricao)
